    ...
    if (show) {
      return (
        <div className='stan-alert-container'>
          <Alert variant={alertType} onClose={() => setShow(false)} dismissible={!autoClose}>
            {alertTitle === '' ? null : <Alert.Heading className='alert-title'>{alertTitle}</Alert.Heading>}
            <div className={`alert-content text-${textAlign}`}>{alertContent}</div>
          </Alert>
        </div>
      );
    }
